...
1. **Tests + lint + format pass** — `npm test` (hermetic; no creds/network), `npm run lint` (0 errors), and `npm run format` (Prettier). All required for any code change; CI enforces them.
...
- The `test` script is scoped to `test/*.test.js`; keep it that way so `node --test` only runs the real suite.
...
- **stdio safety**: stdout is the JSON-RPC channel. **Never log to stdout.** The logger routes non-error levels to stderr; in server-loaded code use `console.error`, never `console.log`.

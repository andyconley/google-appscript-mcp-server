...
export const SCOPES = [
  ...
  // Drive (read-only metadata) — used by list_script_projects to discover
  // Apps Script projects. Adding this scope requires re-consent (re-run
  // oauth-setup.js) before the Drive-backed tools will work.
...
// Token manager instance. Exported as _tokenManager so tests can seed its
// in-memory cache with a fake token (avoids real credentials / network).
const tokenManager = new TokenManager();
export { tokenManager as _tokenManager };
...
function createOAuth2Client() {
  ...
  if (!clientId || !clientSecret) {
    ...
    throw new Error(
      'Missing required OAuth credentials: GOOGLE_APP_SCRIPT_API_CLIENT_ID and GOOGLE_APP_SCRIPT_API_CLIENT_SECRET must be set in environment variables'
    );
  }
  ...
  console.error('   - Scopes:', SCOPES.length, 'permissions');
...
async function startOAuthFlow() {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      console.error(`🌐 Starting OAuth callback server on port ${callbackPort}`);
      console.error(`🔗 Redirect URI: ${REDIRECT_URI}`);

      // Create temporary HTTP server for OAuth callback
      const server = createServer(async (req, res) => {
        console.error('📥 OAuth callback received:', req.url);
        ...
        try {
          ...
          if (url.pathname === '/oauth/callback') {
            ...
            try {
              ...
              console.error('🎟️ Token details:');
              console.error('   - Access token:', newTokens.access_token ? '✅ Received' : '❌ Missing');
              console.error('   - Refresh token:', newTokens.refresh_token ? '✅ Received' : '❌ Missing');
              console.error('   - Token type:', newTokens.token_type || 'Not specified');
              console.error(
                '   - Expires in:',
                newTokens.expiry_date ? new Date(newTokens.expiry_date).toISOString() : 'No expiry'
              );
              console.error('   - Scope:', newTokens.scope || 'Not specified');
      ...
      server.listen(callbackPort, () => {
        console.error(`🌐 OAuth callback server started on port ${callbackPort}`);
        ...
        console.error('🔗 Opening OAuth URL in browser...');
        console.error('📋 OAuth URL:', authUrl);
      ...
      server.on('error', (err) => {
        ...
        if (err.code === 'EADDRINUSE') {
          reject(
            new Error(
              `Port ${callbackPort} is already in use. Please close other applications using this port or update your Google Cloud Console redirect URI to use a different port.`
            )
          );
        } else {
          reject(err);
...
export async function getOAuthAccessToken() {
  ...
  console.error('🔐 Getting OAuth access token...');
...
export async function getAuthHeaders() {
  ...
  console.error('📋 Creating authorization headers...');
  ...
  console.error('✅ Authorization headers created successfully');
  return headers;
}
...
export async function manualOAuthFlow() {
  console.error('🔄 Starting manual OAuth flow...');
...
export function clearTokens() {
  console.error('🚪 Clearing stored tokens...');

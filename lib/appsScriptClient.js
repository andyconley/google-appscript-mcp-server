...
// Per-request timeout (ms). Without this a hung Google call blocks the tool —
// and the stdio server is sequential, so it stalls the whole session.
...
async function fetchWithResilience(url, init, method, tag) {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (isTimeout) throw new Error(`Request timed out after ${REQUEST_TIMEOUT_MS}ms`);
      throw error;
    }
  }
  throw lastError;
